...
async function request(method, url, body, opts = {}) {
  ...
  // CSRF defense: a custom request header that browsers cannot set on a
  // simple cross-origin request without triggering a CORS preflight. The
  // server requires this header (or a matching Origin/Referer) on writes.
  if (method !== 'GET' && method !== 'HEAD') {
    reqOpts.headers['X-Requested-With'] = 'billtracker';
  }
  let res;
  try {
...
export const api = {
  ...
  resetData:      ()      => request('POST', '/api/settings/reset', { confirm: 'reset' })
};
